...
/*   Updated: 2024/09/14 14:26:31 by kishizu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	calc_dis(t_3d_vec hit_point, t_3d_vec shadow_ray, double t)
{
	...
	cross_point = vec_add(hit_point, vec_scalar_mult(shadow_ray, t));
		...
		t_rt_info *game, t_detect_status *st)
{
	...
	while (j < game->sp_num)
	{
		normal = vec_sub(hit_point, game->sphere[j].center_point);
		hit_point_offset = calc_offset(hit_point, normal,
				game->camera.initial_point, 1e-4);
		...
		if (cross_detection_ray_and_sphere(shadow_ray, hit_point_offset,
			...
			&& calc_dis(hit_point, shadow_ray, st->t_sphere) < light_distance)
			return (true);
		j++;
	}
	return (false);
		...
		double light_distance)
{
	...
	while (j < game->cy_num)
	{
		if (cross_detection_ray_and_cylinder(shadow_ray, hit_point,
				&game->cylinder[j], &t) && t > EPSILON && calc_dis(hit_point,
				shadow_ray, t) < light_distance)
			return (true);
		j++;
	}
